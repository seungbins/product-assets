...
3개 리서치 트랙(① 경제성/정책, ② 해커 행동·비교연구, ③ 법률·정책·최신동향)을 통해 웹 검색·원문 확인(arXiv, USENIX, ACM DL, Oxford Academic, SSRN 등)으로 실존을 검증한 논문 **28편**입니다. 각 논문은 제목/저자/게재처/링크와 함께, 빠르게 훑어볼 수 있는 한글 핵심 요약, 그리고 원 조사 내용을 보존한 영문 상세 요약을 담고 있습니다.
...
- 3개 병렬 리서치 트랙(경제성/정책, 해커 행동/비교연구, 법률/정책/최신동향)이 각각 WebSearch + WebFetch로 원문 게재처(USENIX, ACM DL, Oxford Academic, arXiv, SSRN 등)를 직접 확인.
- 검증 실패했거나 동료심사 상태가 불확실한 논문(예: 일부 블로그 포스트, 포스터 논문, 미확인 학위논문)은 제외했으며, 프리프린트는 본문에 명시적으로 표기함.
